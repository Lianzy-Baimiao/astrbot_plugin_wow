...
## v1.1.31 — 整合分颜色按官方分档表取色（2026-09-19）
- [x] 用户实测发现：整合后 3155.7 仍是绿色——score_color 取的是活体档案 2068.7 的 segments 颜色
- [x] 排查：raider.io 颜色是随分数连续渐变（抽样 1709 亮绿/2320 青绿/2688 蓝/2920 深蓝），非离散档；
      官方插件 db/db_score_tiers.lua 内含当前赛季 151 档配色表（MN S2，200→3950）
- [x] charinfo.py：内嵌 SCORE_TIERS 表 + _score_tier_color()；整合分按表取色（3155.7 → epic 紫 #ad38e3），
      未整合角色仍用接口自带 segments.color（官方精确值）
- [x] 表与网站实色有微小偏差（官方表是周更快照、网站曲线持续漂移），色系一致，装饰色可接受
- [x] 打包校验通过，_smoke 归档
