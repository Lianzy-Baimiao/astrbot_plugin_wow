...
| 宠物对战 | `宠物 [详情]` | 预测**明天国服**宠物对战世界任务（重量级野兽等），附国服可做时间区间与上次重量级野兽出现时间；「详情」带奖励明细 |
| 宠物对战 | `宠物推送 开/关/状态/测试` | 本群开启后每天 16:05 检查美服数据，明天国服有**重量级野兽**时自动预警（开/关/测试需管理员） |
...
进入下一天，因此**北京时间 23:00 后拉一次美服当前批次，就是国服明天 07:00 重置后
会出的任务**——国服窗口为次日 07:00–23:00（16 小时）。插件每天 16:05 检查
（美服当天 16 点数据必然已刷新），出现「重量级野兽」（Beasts of Burden，风暴峡湾）
即向开启预警的群推送，文案附国服可做时间区间与上次出现时间。任务中文名内置离线表
（`wow/data/petquests.py`），新任务自动回退英文原名。
