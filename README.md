...
| 宠物对战 | `宠物 [详情]` | 预测国服**明天**宠物对战世界任务（附可做时间区间与上次重量级野兽出现时间）；「详情」带奖励明细 |
| 宠物对战 | `宠物推送 开/关/状态/测试` | 本群开启后每天 16:05 通报国服明天批次，有**重量级野兽**时加预警（开/关/测试需管理员） |
...
持续 24 小时，全部在美服每日重置时刻（北京时间 23:00）结束。**国服比美服晚套用
一批**：每个批次在美服结束后 8 小时，国服才在次日上午 07:00 重置时开始同一批，
所以**美服 16 点（北京早上 7 点）查到的当前批次，就是国服下一天 07:00 开始的
批次**——国服可做窗口 = 批次结束日的次日上午 07:00 起 24 小时。任何时刻查询都是
预测：白天查到的是国服**明天**的批次，夜里（北京 23:00 后）查到的是国服**后天**
的批次（美服刚刷新的新批）。插件每天 16:05 向开启通报的群推送国服明天的批次
（附可做时间区间与上次重量级野兽出现时间），批次里有「重量级野兽」（Beasts of
Burden，风暴峡湾）时加预警横幅。任务中文名内置离线表（`wow/data/petquests.py`），
新任务自动回退英文原名。
