...
| 宠物对战 | `宠物 [详情]` | 国服**今天**宠物对战世界任务（附剩余时间与上次重量级野兽出现时间）；「详情」带奖励明细 |
| 宠物对战 | `宠物推送 开/关/状态/测试` | 本群开启后每天 16:05 通报国服当天批次（剩约 15 小时），有**重量级野兽**时加预警（开/关/测试需管理员） |
...
持续 24 小时，全部在美服每日重置时刻（北京时间 23:00）结束。**国服与美服做的是
同一批任务，只是窗口错位 8 小时**（2026-09-16 实测定稿）：美服批 D-1 日 23:00 ~
D 日 23:00，国服同批 D 日 07:00 ~ D+1 日 07:00（07:00 对齐国服每日重置）。所以
白天查到的 Active 批次 = 国服**今天** 07:00 已开始的批次；夜里（北京 23:00 后，
美服刚刷新批）查到的是国服**明天**的批次。插件每天 16:05 向开启通报的群推送国服
当天的批次（当日通报，剩约 15 小时，附上次重量级野兽出现时间），批次里有「重量级
野兽」（Beasts of Burden，风暴峡湾）时加预警横幅；推送按批次结束日取数，数据源
偶发提前翻页时不会推错。任务中文名内置离线表（`wow/data/petquests.py`），
新任务自动回退英文原名。
